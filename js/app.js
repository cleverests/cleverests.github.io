  ...
  complete: function (anim) {
    setTimeout(function () {
      anime({
        ...
        complete: function (anim) {
          //go through the span elements within the div with id scroll-in1 and add id cleverest+index to each span element
          $("#scroll-in1 span").each(function (index) {
            $(this).attr("id", "cleverest" + (index + 1));
          });

          //add attribute data-scroll to ids cleverest1, cleverest2, cleverest3, cleverest4, cleverest5, cleverest6, cleverest7
          $(
            "#cleverest1, #cleverest2, #cleverest3, #cleverest4, #cleverest5, #cleverest6, #cleverest7"
          ).attr("data-scroll", "true");

          //add attribute data-scroll-position to ids cleverest1, cleverest2, cleverest3, cleverest4, cleverest5, cleverest6, cleverest7
          //$("#cleverest1, #cleverest2, #cleverest3, #cleverest4, #cleverest5, #cleverest6, #cleverest7").attr("data-scroll-position", "top");

          $("#cleverest1").attr("data-scroll-speed", "7");
        },
      });
    }, 500);
    setTimeout(function () {
      ...
      $("head").append('<link rel="stylesheet" href="./css/preview.css"/>');
    }, 1750);
  },
});
...
//       // // cleverest text anim on scroll
//       // let project_timer = setInterval(cleveresttext, 1);
//       // function cleveresttext() {
...
$("#cs1").click(function () {
  ...
  let cs_margin_top =
    $("#cs-all-holder1").offset().top + $(window).height() * 0.028 + 1;
  ...
  $("#cs1")
    ...
    ); // remove empty space below the cs tab
...
$("#cs2").click(function () {
  ...
  let header_margin_top = $("#sticky-project-header").offset().top;
  let cs_margin_top =
    $("#cs-all-holder1").offset().top + 1 + $("#cs1").height();
...
$("#menu")
  .on("mousedown", function () {
    ...
    if ($("#menu").position().top == 0) {
      // checking if the nav menu is at the top of the viewport
      ...
      // the nav menu is at the bottom of the viewport
...
$(document).ready(function () {
  ...
  //mailto:cleverestmedia@gmail.com
  e0 = "cle";//cle
  e1 = "example12";
  e2 = e1.slice(3, 6);//xxx
  e3 = e2.slice(0, 1);//x
  e4 = "xmail";
  e5 = e4.replace("x", "g");//gmail
  e6 = ".com";//.com
...
$(document).ready(function () {
  ...
  e5 = e4.replace("x", "g"); //gmail
  e6 = ".com"; //.com
  var where_to = e0 + e2 + e3 + em + "@" + e5 + e6;
  $("#email3, #email4, #email5, #email6, #build-the-future, #something").on(
    "click",
    function (event) {
      event.preventDefault();
  ...
  // $('#build-the-future').on('click', function (event) {
  //   event.preventDefault();
  //   //document.location.href = "mailto:" + where_to;
  //   e7 = "mailto:" + where_to;
  //   window.open(e7, "_blank");
...
if ($(window).width() > 1024) {
  ...
  function cleveresttext() {
    ...
    //console.log("d = " + d_move);
    d_move = "matrix(1, 0, 0, 1, 0, " + d_move + ")";
    $("#cleverest1").css("transform", d_move);

    let r_move = (($(window).height() - work_offset) / 2) * 1; // eg: 33.33
    //console.log("r = " + r_move);
    r_move = "matrix(1, 0, 0, 1, 0, " + r_move + ")";
    $("#cleverest3").css("transform", r_move);

    let u_move = (($(window).height() - work_offset) / 6) * 1; // eg: 33.33
    //console.log("u = " + u_move);
    u_move = "matrix(1, 0, 0, 1, 0, " + u_move + ")";
    //$("#cleverest2").css("transform", u_move);

    let h_move = (($(window).height() - work_offset) / 4) * 1; // eg: 33.33
    //console.log("h = " + h_move);
    h_move = "matrix(1, 0, 0, 1, 0, " + h_move + ")";
    $("#cleverest4").css("transform", h_move);
    $("#cleverest2").css("transform", h_move);

    let i_move = (($(window).height() - work_offset) / 1) * 1; // eg: 33.33
    //console.log("i = " + i_move);
    i_move = "matrix(1, 0, 0, 1, 0, " + i_move + ")";
    $("#cleverest5").css("transform", i_move);

    let n_move = (($(window).height() - work_offset) / 2) * 1; // eg: 33.33
    //console.log("n = " + n_move);
    n_move = "matrix(1, 0, 0, 1, 0, " + n_move + ")";
    $("#cleverest6").css("transform", n_move);

    let ab_move = (($(window).height() - work_offset) / 1.5) * 1; // eg: 33.33
    //console.log("ab = " + ab_move);
    ab_move = "matrix(1, 0, 0, 1, 0, " + ab_move + ")";
    $("#cleverest7").css("transform", ab_move);
  }
}
